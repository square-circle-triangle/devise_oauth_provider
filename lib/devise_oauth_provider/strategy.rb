        ...
        def authenticate!      
          valid = if params[:action] == 'request_token'
            if verify_oauth_consumer_signature
              ...
              success!(Account.where("lower(username) = ?", "PublisherApp".downcase).first)
            else
              fail!(:invalid)
            end
          elsif params[:action] == 'access_token'
            #success!(@current_client_application.account) if verify_oauth_request_token
            success!(Account.where("lower(username) = ?", "PublisherApp".downcase).first) if verify_oauth_request_token
          else
